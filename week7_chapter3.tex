...
typedef struct Node {
    ...
    struct Node* next; // Liên kết
        ...
        \begin{column}{0.5\textwidth}
            \centering
            \begin{tikzpicture}
                \node[listnode] (N) {value \nodepart{second} next};
                \node[above] at (N.north) {\textbf{Cấu trúc 1 Node}};
                \draw[->] (N.two south) -- ++(0,-0.5) node[below] {Trỏ tới Node kế tiếp};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Chèn phần tử vào ĐẦU danh sách}
    ...
    \begin{enumerate}
        \item Tạo node mới. 
        \item \texttt{new\_node->next = head}. 
        \item Cập nhật \texttt{head = new\_node}.
    \end{enumerate}
...
\begin{frame}[fragile]{Chèn phần tử vào CUỐI danh sách}
    \small
    \textbf{Ý tưởng:} Duyệt đến cuối (\texttt{p->next == NULL}), sau đó trỏ \texttt{p->next} vào node mới.
...
\begin{frame}[fragile]{Chèn vào TRƯỚC một phần tử}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.6\textwidth}
            \begin{minted}{c}
Node* insertBefore(Node* head, Node* p, int v){
...
\begin{frame}[fragile]{Xóa một phần tử (Recursive)}
    \small
    \textbf{Ý tưởng (Đệ quy):} Nếu \texttt{head} là node cần xóa, trả về \texttt{head->next}. Ngược lại, gọi đệ quy cho \texttt{head->next}.
    ...
    return head;
        ...
        \begin{column}{0.4\textwidth}
            ...
            \begin{tikzpicture}[scale=0.75, transform shape]
                ...
                % Link nhảy cóc
                \draw[ptr, ptrColor, thick] (A.south) to[out=-30, in=-150] (C.south);
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Đảo ngược danh sách (Reverse)}
    \small
    \textbf{Ý tưởng:} Dùng 3 con trỏ \texttt{prev}, \texttt{cur}, \texttt{next} để đảo chiều mũi tên.
...
Node* reverse(Node* head) {
    ...
    return prev; // prev là head mới
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8, transform shape]
                \node (NULL) {NULL};
                \node[listnode, right=0.3cm of NULL] (A) {1 \nodepart{second} $\bullet$};
                \node[listnode, right=0.5cm of A] (B) {2 \nodepart{second} $\bullet$};
                
                % Mũi tên gốc (nhạt)
                \draw[->, gray!30] (A.two east) -- (B);
                
                % Mũi tên đảo (đậm)
                \draw[ptr, ptrColor] (A.two south) to[out=200, in=-20] (NULL.south east);
                \node[below=0.1cm of A, font=\tiny] {cur->next=prev};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
